...
export default function Header() {
  ...
  return (
    ...
    >
          ...
          <div className="hidden lg:flex items-center gap-0.5 xl:gap-1">
            {navigation.map((item) => (
              ...
              >
                <Link
                  href={item.href}
                  className={`px-2 xl:px-4 py-2 rounded-full text-sm font-heading font-medium transition-all duration-200 whitespace-nowrap ${
                    isActive(item.href)
                      ? isScrolled
                        ? 'text-violet bg-violet-50'
                        : 'text-white bg-white/15'
                      : isScrolled
                      ? 'text-gray-600 hover:text-violet hover:bg-violet-50'
      ...
      {/* Mobile Menu Panel */}
      <div
        className={`fixed top-0 right-0 h-full w-80 max-w-[calc(100vw-3rem)] bg-white z-40 lg:hidden transform transition-transform duration-300 ease-out shadow-2xl overflow-y-auto ${
                    ...
                    >
                      ...
                      <svg
                        className={`w-4 h-4 transition-transform duration-200 ${
                          mobileDropdown === item.titre ? 'rotate-180' : ''
                        }`}
