...
export default function Header() {
  ...
  return (
    ...
    >
        ...
        <nav className="flex items-center justify-between h-16 sm:h-20">
          ...
          <div className="hidden lg:flex items-center gap-0.5 xl:gap-1">
            {navigation.map((item) => (
              ...
              >
                {item.enfants ? (
                  <button onClick={() => setDropdownOpen(dropdownOpen === item.titre ? null : item.titre)} className={linkClass(item.href)}>
                    {item.titre}
                  </button>
                ) : (
                ...
                {item.enfants && dropdownOpen === item.titre && (
                  <div className="absolute top-full left-0 mt-1 w-52 bg-white rounded-xl shadow-lg border border-gray-100 py-2 animate-fade-in z-50">
                    {item.enfants.map((enfant) => (
                      <Link key={enfant.href} href={enfant.href} onClick={() => setDropdownOpen(null)} className="block px-4 py-2.5 text-sm text-gray-600 hover:text-violet hover:bg-violet-50 transition-colors">
                        {enfant.titre}
                      </Link>
                    ))}
                  </div>
            ...
            <Link href="/contact" className="ml-2 xl:ml-4 px-4 xl:px-6 py-2 xl:py-2.5 bg-violet text-white font-heading font-semibold text-sm rounded-full hover:bg-violet-700 transition-all shadow-sm">
              Contact
            </Link>
          </div>

          {/* Mobile Burger */}
          <button onClick={() => setMobileOpen(!mobileOpen)} className="lg:hidden z-[101] p-2 -mr-2" aria-label="Menu" aria-expanded={mobileOpen}>
      ...
      {/* Mobile Overlay */}
      <div className={`fixed inset-0 bg-black/30 z-40 lg:hidden transition-opacity duration-300 ${mobileOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} onClick={() => setMobileOpen(false)} />

      {/* Mobile Panel */}
      <div className={`fixed top-0 right-0 h-full w-80 max-w-[calc(100vw-3rem)] bg-white z-40 lg:hidden transform transition-transform duration-300 ease-out shadow-2xl overflow-y-auto ${mobileOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <nav className="flex flex-col gap-1">
            {navigation.map((item) => (
              <div key={item.titre} className="border-b border-gray-50 last:border-0">
                {item.enfants ? (
                  <>
                    <button onClick={() => setMobileDropdown(mobileDropdown === item.titre ? null : item.titre)} className="w-full flex items-center justify-between py-3.5 text-base font-heading font-semibold text-gray-900 hover:text-violet transition-colors">
                      {item.titre}
                    ...
                    <div className={`overflow-hidden transition-all duration-200 ${mobileDropdown === item.titre ? 'max-h-60 pb-2' : 'max-h-0'}`}>
                      {item.enfants.map((enfant) => (
