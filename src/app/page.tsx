        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-24 lg:py-32">
          <div className="max-w-3xl mx-auto text-center">
            <div className="mb-6 sm:mb-10 animate-fade-in">
              <img src="/logo/kja-studio-labs.jpg" alt="KJA Studio Labs" className="h-12 sm:h-16 lg:h-20 mx-auto" />
            </div>
            <h1 className="font-heading font-bold text-white text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl leading-[1.1] mb-4 sm:mb-6 animate-slide-up">
              Design{' '}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-200 to-white">UI/UX</span>
              {' '}& Développement{' '}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-violet-200">Fullstack</span>
            </h1>
            ...
            <>
                  ...
                  <Link key={projet.id} href={`/portfolio/${projet.slug}`} className="card overflow-hidden group block">
                    <div className="aspect-video bg-gray-100 flex items-center justify-center overflow-hidden">
                      {projet.imagePrincipale ? (
                        <img src={projet.imagePrincipale} alt={projet.titre} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                      ) : (
              ...
              {totalProjets > 3 && (
                <div className="text-center">
                  <Link href="/portfolio" className="btn-secondary text-sm sm:text-base px-6 sm:px-8 py-3 sm:py-4">Voir tous les projets ({totalProjets})</Link>
                </div>
              )}
            </>
          ) : (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {articlesRecents.length > 0 ? (
            <>
                  ...
                  <Link key={article.id} href={`/blog/${article.slug}`} className="card overflow-hidden group block bg-white">
                    ...
                    <div className="p-5 lg:p-6">
                      <div className="flex items-center gap-3 mb-2">
                        <span className="text-xs font-heading font-semibold text-violet uppercase tracking-wider">{blogCategoriesLabels[article.categorie] || article.categorie}</span>
                        {article.tempsLecture && <span className="text-xs text-gray-400">{article.tempsLecture} min de lecture</span>}
                      </div>
              ...
              {totalArticles > 3 && (
                <div className="text-center">
                  <Link href="/blog" className="btn-secondary text-sm sm:text-base px-6 sm:px-8 py-3 sm:py-4">Voir tous les articles ({totalArticles})</Link>
                </div>
              )}
            </>
          ) : (
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex gap-4 sm:gap-6 animate-scroll-fast">
              {[...avisRecents, ...avisRecents, ...avisRecents].map((avis, index) => (
                <div key={`${avis.id}-${index}`} className="card p-4 sm:p-8 shrink-0 min-w-[280px] sm:w-[380px]">
                  ...
                  <p className="text-sm sm:text-base text-gray-600 leading-relaxed mb-5 italic">&ldquo;{avis.commentaire}&rdquo;</p>
                  <div className="border-t border-gray-100 pt-4">
                    <p className="font-heading font-semibold text-gray-900 text-sm sm:text-base">{avis.nomClient}</p>
                    {(avis.poste || avis.entreprise) && (
                      <p className="text-xs sm:text-sm text-gray-500 mt-0.5">{avis.poste}{avis.poste && avis.entreprise && ' chez '}{avis.entreprise}</p>
                    )}
                  </div>
                </div>
              ))}
            </div>
            {avisRecents.length >= 10 && (
              <div className="text-center mt-10">
                <Link href="/avis" className="btn-secondary text-sm sm:text-base px-6 sm:px-8 py-3 sm:py-4">Voir tous les avis</Link>
              </div>
            )}
          </div>
        </section>
