...
export default async function DashboardPage() {
  const [totalProjets, totalArticles, totalAvis, avisEnAttente, messagesNonLus] = await Promise.all([
    prisma.projet.count(),
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
        {stats.map((stat) => (
          <Link
            key={stat.label}
            href={stat.href}
            className={`${stat.couleur} rounded-2xl p-5 sm:p-6 hover:shadow-md transition-shadow`}
          >
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/admin/projets" className="text-sm text-violet hover:underline">
              Voir tout
            </Link>
          </div>
          <ProjetsRecents />
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/admin/blog" className="text-sm text-violet hover:underline">
              Voir tout
            </Link>
          </div>
          <ArticlesRecents />
...
async function ProjetsRecents() {
  ...
  if (projets.length === 0) {
    return <p className="text-gray-500 text-sm">Aucun projet.</p>;
  }
      ...
      {projets.map((p) => (
        <li key={p.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
          ...
          <span className={`text-xs px-2 py-1 rounded-full font-medium ${
            p.estPublie ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-500'
          }`}>
            {p.estPublie ? 'Publié' : 'Brouillon'}
          </span>
        </li>
      ))}
    </ul>
...
async function ArticlesRecents() {
  ...
  if (articles.length === 0) {
    return <p className="text-gray-500 text-sm">Aucun article.</p>;
  }
    ...
    <ul className="space-y-3">
      {articles.map((a) => (
        <li key={a.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
          ...
          <span className={`text-xs px-2 py-1 rounded-full font-medium ${
            a.estPublie ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-500'
          }`}>
